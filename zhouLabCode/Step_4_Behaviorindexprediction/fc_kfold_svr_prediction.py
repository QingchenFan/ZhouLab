...
def PLSPrediction_Model(data_list, dimention, weightpath, Permutation,kfold, datamark,Time=1):
    ...
    for train_index, test_index in kf.split(x_data):
        ...
        cv_time = 2
        predict_model = GridSearchCV(Lsvr, param_grid=param_grid, verbose=6, cv=cv_time)
        predict_model.fit(Subjects_Data_train, y_train)
        best_model = predict_model.best_estimator_
        # weight
        feature_weight = best_model.coef_
        print('--feature_weight_res-shape--', feature_weight_res.shape)
        print('--feature_weight-shape--', feature_weight.shape)

        feature_weight_res = np.add(feature_weight_res, feature_weight)
        print('--feature_weight_res--',feature_weight_res)
        print('--feature_weight_res-shape--',feature_weight_res.shape)
    ...
    print('--feature_weight_res_mean--',feature_weight_res_mean)
    print('--feature_weight_res_mean-shape--',feature_weight_res_mean.shape)

    feature_weight_file = pd.DataFrame(feature_weight_res_mean)
    print('--feature_weight_file--', feature_weight_file)
    if Permutation:
